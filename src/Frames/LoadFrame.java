...
public class LoadFrame extends JFrame implements ActionListener {
    ...
    public LoadFrame(Coin coin, Storage storage, Home home, Statistics statistics, Settings settings) {
        ...
        ImageIcon backgroundImage = new ImageIcon(getClass().getResource("/Image/"+settings.updateTheme()+"/Load.png"));
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == button1) {
            SaveManage.loadGame(coin, home.getProductsOnField(), storage.getProducts(),settings,statistics,1);
            JOptionPane.showMessageDialog(this, "Load Complete.");
            home.loadGame();
            this.dispose();
        }
        if (e.getSource() == button2) {
            SaveManage.loadGame(coin, home.getProductsOnField(), storage.getProducts(),settings,statistics,2);
            JOptionPane.showMessageDialog(this, "Load Complete.");
            home.loadGame();
            this.dispose();
        }
        if (e.getSource() == button3) {
            SaveManage.loadGame(coin, home.getProductsOnField(), storage.getProducts(),settings, statistics,3);
            JOptionPane.showMessageDialog(this, "Load Complete.");
            home.loadGame();
            this.dispose();
        }
